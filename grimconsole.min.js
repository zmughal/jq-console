...
_ref=this.matchings.clss;for(_i=0,_len=_ref.length;_i<_len;_i++){cls=_ref[_i];$("."+cls,this.$console).contents().unwrap()}if(config=this.matchings.closings[current_char]){found=this._ProcessMatch(config,true,before_char)}else if(config=this.matchings.openings[current_char]){found=this._ProcessMatch(config,false,before_char)}else if(!before_char){this._CheckMatchings(true)}if(before_char){if(found){return this._Wrap(this.$prompt_left,this.$prompt_left.html().length-1,config.cls)}}else{if(found){return this._Wrap(this.$prompt_right,0,config.cls)}}};JQConsole.prototype._HistoryPrevious=function(){if(!this.history_active){return}if(this.history_index<=0){return}if(this.history_index===this.history.length){this.history_new=this.GetPromptText()}return this.SetPromptText(this.history[--this.history_index])};JQConsole.prototype._HistoryNext=function(){if(!this.history_active){return}if(this.history_index>=this.history.length){return}if(this.history_index===this.history.length-1){this.history_index++;return this.SetPromptText(this.history_new)}else{return this.SetPromptText(this.history[++this.history_index])}};JQConsole.prototype._CheckComposition=function(e){var key;key=e.keyCode||e.which;if($.browser.opera!=null&&this.in_composition){this._UpdateComposition()}if(key===229){if(this.in_composition){return this._UpdateComposition()}else{return this._StartComposition()}}};JQConsole.prototype._StartComposition=function(){this.$input_source.bind(E_KEYPRESS,this._EndComposition);this.in_composition=true;this._ShowComposition();return setTimeout(this._UpdateComposition,0)};JQConsole.prototype._EndComposition=function(){this.$input_source.unbind(E_KEYPRESS,this._EndComposition);this.in_composition=false;this._HideComposition();return this.$input_source.val("")};JQConsole.prototype._UpdateComposition=function(e){var cb,_this=this;cb=function(){if(!_this.in_composition){return}return _this.$composition.text(_this.$input_source.val())};return setTimeout(cb,0)};JQConsole.prototype._ShowComposition=function(){this.$composition.css("height",this.$prompt_cursor.height());this.$composition.empty();return this.$composition.appendTo(this.$prompt_left)};JQConsole.prototype._HideComposition=function(){return this.$composition.detach()};return JQConsole}();$.fn.jqconsole=function(header,prompt_main,prompt_continue){return new JQConsole(this,header,prompt_main,prompt_continue)};$.fn.jqconsole.JQConsole=JQConsole;$.fn.jqconsole.Ansi=Ansi;$=jQuery;KEY_N=parseInt("N");KEY_P=parseInt("P");KEY_T=parseInt("T");KEY_W=parseInt("W");KEY_U=parseInt("U");KEY_ESC=27;PROMPT_MARKER_ID="prompt-completion-marker";PROMPT_INPUT_ID="prompt-completion-input";GrimConsole=function(_super){__extends(GrimConsole,_super);function GrimConsole(container,header,prompt_label,prompt_continue_label){this.container=container;this._HandleChar=__bind(this._HandleChar,this);this._HandleKey=__bind(this._HandleKey,this);GrimConsole.__super__.constructor.call(this,this.container,"Welcome to Grimoire\n","grim> ",".");this.completion_callback=null;this.$is_completion=false;this.$is_completion_can_end=false}GrimConsole.prototype.Reset=function(){this.completion_callback=null;this.EndCompletion();return GrimConsole.__super__.Reset.apply(this,arguments)};GrimConsole.prototype.BackspaceToBegin=function(){var text;setTimeout($.proxy(this._ScrollToEnd,this),0);text=this.$prompt_left.text();if(text){return this.$prompt_left.text("")}else{return this._Backspace(false)}};GrimConsole.prototype.DeleteToEnd=function(){var text;text=this.$prompt_right.text();if(text){return this.$prompt_right.text("")}else{return this._Delete(false)}};GrimConsole.prototype._HandleKey=function(event){var key;key=event.keyCode||event.which;if(event.ctrlKey){if(key in this.shortcuts){event.preventDefault()}}if(key===KEY_TAB){if(event.shiftKey){if(this.$is_completion){event.preventDefault();this.SelectPreviousCompletion();return}else{GrimConsole.__super__._HandleKey.apply(this,arguments);return}}else{if(this.$is_completion){event.preventDefault();if(this.$is_completion_can_end){this.EndCompletion();this.Complete()}else{this.SelectNextCompletion()}return}else{event.preventDefault();if(this.ShouldIndent()){GrimConsole.__super__._HandleKey.apply(this,arguments)}else{this.Complete()}return}}}else if(key===KEY_BACKSPACE){if(this.ShouldDeindent()){this._Unindent()}else{GrimConsole.__super__._HandleKey.apply(this,arguments)}if(this.$is_completion){this.UpdateCompletionMenu()}return}else if(key===KEY_ESC){if(this.$is_completion){this.EndCompletion();return}}else if(key===KEY_ENTER){if(this.$is_completion){this.AcceptCompletion();return}}GrimConsole.__super__._HandleKey.apply(this,arguments);if(this.$is_completion){return this.UpdateCompletionMenu()}};GrimConsole.prototype._HandleChar=function(event){GrimConsole.__super__._HandleChar.apply(this,arguments);if(this.$is_completion){return this.UpdateCompletionMenu()}};GrimConsole.prototype.AcceptCompletion=function(){var current_text;current_text=this.GetCompletionItemSelected();this.$prompt_left.text(this.$prompt_left.text().substring(0,this.$completion_start_idx));this._AppendPromptText(current_text.substring(this.$completion_prefix_idx));return this.EndCompletion()};GrimConsole.prototype.GetCompletionItemSelected=function(){var items;items=this.GetCompletionItems();if(this.$prompt_current_idx>=0&&this.$prompt_current_idx<items.length){return $(items[this.$prompt_current_idx]).text()}return""};GrimConsole.prototype.GetCompletionItems=function(){return $(this.$prompt_completion_input.autocomplete("widget").find("a"))};GrimConsole.prototype.SelectPreviousCompletion=function(){var items;items=this.GetCompletionItems();if(this.$prompt_current_idx===-1){this.$prompt_current_idx=items.length}if(this.$prompt_current_idx>0){this.$prompt_current_idx--}else{this.$prompt_current_idx=items.length-1}return this.SelectNthCompletion(this.$prompt_current_idx)};GrimConsole.prototype.SelectNextCompletion=function(){var items;items=this.GetCompletionItems();if(this.$prompt_current_idx<items.length-1){this.$prompt_current_idx++}else{this.$prompt_current_idx=0}return this.SelectNthCompletion(this.$prompt_current_idx)};GrimConsole.prototype.SelectNthCompletion=function(n){var items;items=this.GetCompletionItems();items.removeClass("ui-state-focus");return $(items[n]).addClass("ui-state-focus")};GrimConsole.prototype.UpdateCompletionMenu=function(){if(this.$prompt_left.text().length<this.$completion_start_idx||this.$prompt_left.text().length===0){this.EndCompletion();return}this.$prompt_completion_input.val(this.$prompt_left.text().substring(this.$completion_start_idx-this.$completion_prefix_idx));return this.$prompt_completion_input.autocomplete("search")};GrimConsole.prototype.SetCompletionCallback=function(completion_callback){return this.completion_callback=completion_callback};GrimConsole.prototype.SetupCompletion=function(results){var match,orig_text,pm_input_html,text_after_space,text_before_space,_this=this;orig_text=this.$prompt_left.text();match=orig_text.match(/\b([A-Za-z0-9_:]*)$/);text_before_space=orig_text.substr(0,match.index);text_after_space=orig_text.substr(match.index);this.$prompt_left.text(text_before_space);this.$prompt_completion_mark=$("<span id="+'"'+PROMPT_MARKER_ID+'"'+'style="position: relative"></span>').appendTo(this.$prompt_left);pm_input_html='<input type="text" id='+'"'+PROMPT_INPUT_ID+'"'+" />";this.$prompt_completion_input=$(pm_input_html);this.$prompt_completion_input.css({display:"none"});this.$prompt_current_idx=-1;this.$prompt_completion_input.autocomplete({source:function(request,response){var matches;matches=$.grep(results,function(item,item_ndx){return item.indexOf(request.term)===0});if(matches.length===0){_this.$is_completion_can_end=true}return response(matches)},position:{of:"#"+PROMPT_MARKER_ID},minLength:0});this.$prompt_completion_input.val("");this.$prompt_completion_input.autocomplete("search");this.$prompt_completion_input.autocomplete("close");this.$prompt_completion_input.autocomplete("search");this.$prompt_completion_input.val(text_after_space);this.$prompt_left.text(orig_text);this.$completion_start_idx=this.$prompt_left.text().length;return this.$completion_prefix_idx=text_after_space.length};GrimConsole.prototype.BreakdownCompletion=function(){this.$prompt_completion_mark.remove();this.$prompt_completion_input.remove();this.$completion_start_idx=-1};GrimConsole.prototype.Complete=function(){var text,_this=this;this.$is_completion=true;text=this.$prompt_left.text();return this.completion_callback(text,function(completion_results){if(completion_results.length){return _this.SetupCompletion(completion_results)}else{return _this.EndCompletion()}})};GrimConsole.prototype.InsertCompletion=function(text){this._AppendPromptText(text);return this.EndCompletion()};GrimConsole.prototype.EndCompletion=function(){this.$is_completion=false;this.$is_completion_can_end=false;this.BreakdownCompletion()};GrimConsole.prototype.ShouldIndent=function(){return this.$prompt_left.text().match(/^\s*$/)};GrimConsole.prototype.ShouldDeindent=function(){return this.$prompt_left.text().length>0&&this.$prompt_left.text().match(/^\s*$/)&&this.$prompt_left.text().length%DEFAULT_INDENT_WIDTH===0};return GrimConsole}(JQConsole);$.fn.grimconsole=function(header,prompt_main,prompt_continue){return new grimConsole(this,header,prompt_main,prompt_continue)};$.fn.grimconsole.GrimConsole=GrimConsole;$.fn.grimconsole.Ansi=Ansi}.call(this);