...
(function() {
  var $, Ansi, CLASS_ANSI, CLASS_BLURRED, CLASS_CURSOR, CLASS_HEADER, CLASS_INPUT, CLASS_OLD_PROMPT, CLASS_PREFIX, CLASS_PROMPT, DEFAULT_INDENT_WIDTH, DEFAULT_PROMPT_CONINUE_LABEL, DEFAULT_PROMPT_LABEL, EMPTY_DIV, EMPTY_SELECTOR, EMPTY_SPAN, ESCAPE_CHAR, ESCAPE_SYNTAX, E_KEYPRESS, GrimConsole, JQConsole, KEY_BACKSPACE, KEY_DELETE, KEY_DOWN, KEY_END, KEY_ENTER, KEY_ESC, KEY_HOME, KEY_LEFT, KEY_N, KEY_P, KEY_PAGE_DOWN, KEY_PAGE_UP, KEY_RIGHT, KEY_T, KEY_TAB, KEY_U, KEY_UP, KEY_W, NEWLINE, PROMPT_INPUT_ID, PROMPT_MARKER_ID, STATE_INPUT, STATE_OUTPUT, STATE_PROMPT, spanHtml,
  ...
  Ansi = (function() {
    ...
    function Ansi() {
      ...
      this._append = __bind(this._append, this);      this.klasses = [];
    }
  ...
  GrimConsole = (function(_super) {
    __extends(GrimConsole, _super);

    function GrimConsole(container, header, prompt_label, prompt_continue_label) {
      this.container = container;
      this._HandleChar = __bind(this._HandleChar, this);
      this._HandleKey = __bind(this._HandleKey, this);
      GrimConsole.__super__.constructor.call(this, this.container, "Welcome to Grimoire\n", "grim> ", ".");
      this.completion_callback = null;
      this.$is_completion = false;
      this.$is_completion_can_end = false;
    }

    GrimConsole.prototype.Reset = function() {
      this.completion_callback = null;
      this.EndCompletion();
      return GrimConsole.__super__.Reset.apply(this, arguments);
    };

    GrimConsole.prototype.BackspaceToBegin = function() {
      var text;
      ...
      if (text) {
        ...
        return this._Backspace(false);
      }
    };

    GrimConsole.prototype.DeleteToEnd = function() {
      var text;
    ...
    GrimConsole.prototype._HandleKey = function(event) {
      var key;
      ...
      if (key === KEY_TAB) {
        if (event.shiftKey) {
          if (this.$is_completion) {
            ...
            GrimConsole.__super__._HandleKey.apply(this, arguments);
            return;
          }
        } else {
          if (this.$is_completion) {
            event.preventDefault();
            if (this.$is_completion_can_end) {
              this.EndCompletion();
              this.Complete();
            } else {
        ...
        if (this.ShouldDeindent()) {
          this._Unindent();
        } else {
          GrimConsole.__super__._HandleKey.apply(this, arguments);
        }
        if (this.$is_completion) {
          this.UpdateCompletionMenu();
        }
        return;
      } else if (key === KEY_ESC) {
        if (this.$is_completion) {
      ...
      GrimConsole.__super__._HandleKey.apply(this, arguments);
      if (this.$is_completion) {
        return this.UpdateCompletionMenu();
      }
    };

    GrimConsole.prototype._HandleChar = function(event) {
      GrimConsole.__super__._HandleChar.apply(this, arguments);
      if (this.$is_completion) {
        return this.UpdateCompletionMenu();
      }
    };

    GrimConsole.prototype.AcceptCompletion = function() {
      var current_text;

      current_text = this.GetCompletionItemSelected();
      this.$prompt_left.text(this.$prompt_left.text().substring(0, this.$completion_start_idx));
      this._AppendPromptText(current_text.substring(this.$completion_prefix_idx));
      return this.EndCompletion();
    };

    GrimConsole.prototype.GetCompletionItemSelected = function() {
      var items;

      items = this.GetCompletionItems();
      if (this.$prompt_current_idx >= 0 && this.$prompt_current_idx < items.length) {
        return $(items[this.$prompt_current_idx]).text();
      }
      return '';
    };

    GrimConsole.prototype.GetCompletionItems = function() {
      return $(this.$prompt_completion_input.autocomplete("widget").find("a"));
    };

    GrimConsole.prototype.SelectPreviousCompletion = function() {
      var items;

      items = this.GetCompletionItems();
      if (this.$prompt_current_idx === -1) {
        this.$prompt_current_idx = items.length;
      }
      if (this.$prompt_current_idx > 0) {
        this.$prompt_current_idx--;
      } else {
        this.$prompt_current_idx = items.length - 1;
      }
      return this.SelectNthCompletion(this.$prompt_current_idx);
    };

    GrimConsole.prototype.SelectNextCompletion = function() {
      var items;

      items = this.GetCompletionItems();
      if (this.$prompt_current_idx < items.length - 1) {
        this.$prompt_current_idx++;
      } else {
        this.$prompt_current_idx = 0;
      }
      return this.SelectNthCompletion(this.$prompt_current_idx);
    };

    GrimConsole.prototype.SelectNthCompletion = function(n) {
      var items;

      items = this.GetCompletionItems();
      items.removeClass("ui-state-focus");
      return $(items[n]).addClass("ui-state-focus");
    };

    GrimConsole.prototype.UpdateCompletionMenu = function() {
      if (this.$prompt_left.text().length < this.$completion_start_idx || this.$prompt_left.text().length === 0) {
        this.EndCompletion();
        return;
      }
      this.$prompt_completion_input.val(this.$prompt_left.text().substring(this.$completion_start_idx - this.$completion_prefix_idx));
      return this.$prompt_completion_input.autocomplete('search');
    };

    GrimConsole.prototype.SetCompletionCallback = function(completion_callback) {
      return this.completion_callback = completion_callback;
    };

    GrimConsole.prototype.SetupCompletion = function(results) {
      var match, orig_text, pm_input_html, text_after_space, text_before_space,
        _this = this;

      orig_text = this.$prompt_left.text();
      ...
      this.$prompt_left.text(text_before_space);
      this.$prompt_completion_mark = $('<span id=' + '"' + PROMPT_MARKER_ID + '"' + 'style="position: relative"></span>').appendTo(this.$prompt_left);
      pm_input_html = '<input type="text" id=' + '"' + PROMPT_INPUT_ID + '"' + ' />';
      this.$prompt_completion_input = $(pm_input_html);
      this.$prompt_completion_input.css({
        display: 'none'
      });
      this.$prompt_current_idx = -1;
      this.$prompt_completion_input.autocomplete({
        source: function(request, response) {
          var matches;

          matches = $.grep(results, function(item, item_ndx) {
            return item.indexOf(request.term) === 0;
          });
          if (matches.length === 0) {
            _this.$is_completion_can_end = true;
          }
      ...
      this.$prompt_completion_input.val('');
      this.$prompt_completion_input.autocomplete('search');
      this.$prompt_completion_input.autocomplete('close');
      this.$prompt_completion_input.autocomplete('search');
      this.$prompt_completion_input.val(text_after_space);
      this.$prompt_left.text(orig_text);
      this.$completion_start_idx = this.$prompt_left.text().length;
      return this.$completion_prefix_idx = text_after_space.length;
    };

    GrimConsole.prototype.BreakdownCompletion = function() {
      this.$prompt_completion_mark.remove();
      this.$prompt_completion_input.remove();
      this.$completion_start_idx = -1;
    };

    GrimConsole.prototype.Complete = function() {
      var text,
        _this = this;

      this.$is_completion = true;
      text = this.$prompt_left.text();
      return this.completion_callback(text, function(completion_results) {
        if (completion_results.length) {
          return _this.SetupCompletion(completion_results);
        } else {
          return _this.EndCompletion();
        }
      });
    };

    GrimConsole.prototype.InsertCompletion = function(text) {
      this._AppendPromptText(text);
      return this.EndCompletion();
    };

    GrimConsole.prototype.EndCompletion = function() {
      this.$is_completion = false;
      this.$is_completion_can_end = false;
      this.BreakdownCompletion();
    };

    GrimConsole.prototype.ShouldIndent = function() {
      return this.$prompt_left.text().match(/^\s*$/);
    };

    GrimConsole.prototype.ShouldDeindent = function() {
      return this.$prompt_left.text().length > 0 && this.$prompt_left.text().match(/^\s*$/) && (this.$prompt_left.text().length % DEFAULT_INDENT_WIDTH) === 0;
    };
  ...
  $.fn.grimconsole = function(header, prompt_main, prompt_continue) {
    return new GrimConsole(this, header, prompt_main, prompt_continue);
  };
